...
const PlanningSettings = () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: 'De planning instellingen zijn bijgewerkt.',
      });
      setHasChanges(false);
    },
    onError: (error: any) => {
  ...
  const updateSetting = <K extends keyof PlanningSettingsData>(
    key: K, 
    value: PlanningSettingsData[K]
  ) => {
    if (!settings) return;
  ...
  if (isLoading || !settings) {
    return (
      <DashboardLayout title="Planning Instellingen" description="Configureer automatische goedkeuring, surge pricing en meer">
  ...
  return (
    <DashboardLayout 
      title="Planning Instellingen" 
      description="Configureer automatische goedkeuring, surge pricing en annuleringsbeleid"
    >
      <div className="space-y-4 md:space-y-6 max-w-3xl">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <CardDescription className="text-xs md:text-sm">
                  Keur aanmeldingen automatisch goed
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <div className="flex items-start sm:items-center justify-between gap-3">
              <div className="space-y-0.5 min-w-0">
                ...
                <p className="text-xs md:text-sm text-muted-foreground">
                  Chauffeurs met hoge scores automatisch goedkeuren
                </p>
              </div>
              <Switch
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3 md:gap-4">
                    <Input
                      ...
                      min={0}
                      max={100}
                      value={settings.auto_approve_min_score}
                      onChange={(e) => updateSetting('auto_approve_min_score', parseInt(e.target.value) || 0)}
                      className="w-20 md:w-24 h-9"
                    />
                    <div className="flex-1">
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div 
                          className="h-full bg-green-500 transition-all"
                          style={{ width: `${settings.auto_approve_min_score}%` }}
                        />
                      </div>
                    </div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Score van {settings.auto_approve_min_score}+ wordt automatisch goedgekeurd
                  </p>
                </div>
              </div>
            )}
          ...
          <CardContent className="space-y-4 md:space-y-6">
            <div className="flex items-start sm:items-center justify-between gap-3">
              <div className="space-y-0.5 min-w-0">
                ...
                <p className="text-xs md:text-sm text-muted-foreground">
                  Automatisch bonus toevoegen
                </p>
              </div>
              <Switch
                checked={settings.enable_surge_pricing}
                onCheckedChange={(checked) => updateSetting('enable_surge_pricing', checked)}
              />
            </div>
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      min={1}
                      max={168}
                      value={settings.surge_hours_threshold}
                      onChange={(e) => updateSetting('surge_hours_threshold', parseInt(e.target.value) || 24)}
                      className="h-9"
                    />
                    <p className="text-xs text-muted-foreground hidden md:block">
                      Binnen {settings.surge_hours_threshold}u
                    </p>
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      min={0}
                      max={100}
                      value={settings.surge_bonus_percentage}
                      onChange={(e) => updateSetting('surge_bonus_percentage', parseInt(e.target.value) || 0)}
                      className="h-9"
                    />
                    <p className="text-xs text-muted-foreground hidden md:block">
                      +{settings.surge_bonus_percentage}% extra
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={1}
                  max={168}
                  value={settings.early_cancel_hours}
                  onChange={(e) => updateSetting('early_cancel_hours', parseInt(e.target.value) || 24)}
                  className="h-9"
                />
                <p className="text-xs text-muted-foreground">
                  Binnen {settings.early_cancel_hours}u
                </p>
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    min={0}
                    max={100}
                    value={settings.early_cancel_penalty}
                    onChange={(e) => updateSetting('early_cancel_penalty', parseInt(e.target.value) || 0)}
                    className="h-9 pr-14"
                  />
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    min={0}
                    max={100}
                    value={settings.no_show_penalty}
                    onChange={(e) => updateSetting('no_show_penalty', parseInt(e.target.value) || 0)}
                    className="h-9 pr-14"
                  />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">punten</span>
                </div>
          ...
          <CardHeader className="pb-3 md:pb-6">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <CardDescription className="text-xs md:text-sm">
                  Instellingen voor nieuwe shifts
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4 md:space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              <div className="space-y-2">
                ...
                <Select
                  value={settings.default_compensation_type}
                  onValueChange={(value) => updateSetting('default_compensation_type', value as CompensationType)}
                >
              ...
              <div className="flex items-start sm:items-center justify-between gap-3">
                <div className="space-y-0.5 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Zichtbaar bij nieuwe shifts
                  </p>
                </div>
                <Switch
                  checked={settings.default_show_compensation}
                  onCheckedChange={(checked) => updateSetting('default_show_compensation', checked)}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={settings.require_approval_above_amount || ''}
                  onChange={(e) => updateSetting('require_approval_above_amount', e.target.value ? parseInt(e.target.value) : null)}
                  className="h-9"
                />
                <p className="text-xs text-muted-foreground">
                  Boven dit bedrag: handmatige goedkeuring
                </p>
              </div>
              ...
              <div className="flex items-start sm:items-center justify-between gap-3">
                <div className="space-y-0.5 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Geen auto-approve bij verlies
                  </p>
                </div>
                <Switch
                  checked={settings.require_approval_for_negative_margin}
                  onCheckedChange={(checked) => updateSetting('require_approval_for_negative_margin', checked)}
                />
              </div>
            </div>
          </CardContent>
        ...
        <div className="flex justify-end gap-2 pt-4 sticky bottom-4 md:static bg-background/95 backdrop-blur-sm p-3 -mx-3 md:mx-0 md:p-0 md:bg-transparent rounded-lg md:rounded-none border md:border-0 shadow-lg md:shadow-none">
          <Button variant="outline" onClick={handleReset} disabled={!hasChanges} className="flex-1 md:flex-none">
            Ongedaan maken
          </Button>
          <Button onClick={handleSave} disabled={!hasChanges || saveMutation.isPending} className="flex-1 md:flex-none">
            {saveMutation.isPending ? (
              <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
            ) : (
              <Save className="h-4 w-4 mr-2" />
