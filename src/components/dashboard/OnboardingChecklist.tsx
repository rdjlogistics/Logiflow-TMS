...
export default function OnboardingChecklist() {
  ...
  // Auto-mark complete and hide after 5s
  useEffect(() => {
          ...
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                Je hebt alle stappen voltooid. LogiFlow is klaar voor gebruik!
              </p>
            </div>
        ...
        <CardContent className="p-6">
          ...
          <div className="flex items-start justify-between gap-4 mb-1">
            ...
            <Button
              ...
              aria-label={isMinimized ? "Uitklappen" : "Inklappen"}
            >
...
function StepItem({ step }: { step: OnboardingStep }) {
  ...
  const content = (
    ...
    >
      ...
      {!step.completed && (
        <ChevronRight className="h-4 w-4 text-muted-foreground opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all shrink-0 mt-0.5" />
      )}
    </div>
  );
