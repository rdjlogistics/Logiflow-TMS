...
const FIELD_LABELS: Record<string, string> = {
  ...
  credit_blocked: 'Krediet geblokkeerd',
  credit_blocked_reason: 'Blokkeringsreden',
  payment_terms_days: 'Betalingstermijn',
  payment_method: 'Betaalwijze',
  auto_invoice_enabled: 'Auto-facturatie',
  invoice_type: 'Factuurtype',
  send_invoice_digitally: 'Digitaal versturen',
  invoice_notes: 'Factuurnotities',
  invoice_per_order: 'Factuur per order',
  show_only_subtotals: 'Alleen subtotalen',
  hide_destinations: 'Bestemmingen verbergen',
  deleted_at: 'Verwijderd op',
  customer_number: 'Klantnummer',
};

export const CustomerAuditLogTab = ({ customerId }: { customerId: string }) => {
  ...
  const renderChangeSummary = (entry: AuditLogEntry) => {
    if (entry.action === 'INSERT') return 'Klant aangemaakt';
          ...
          <TableBody>
            ...
            {!isLoading && (!logs || logs.length === 0) && (
              <TableEmpty
                ...
                title="Geen historie"
                description="Er zijn nog geen wijzigingen gelogd voor deze klant"
                colSpan={5}
              />
            )}
            {logs?.map(entry => {
