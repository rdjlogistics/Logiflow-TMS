...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!orderId || !to) return new Response(JSON.stringify({ error: "orderId en to verplicht" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    console.log(`[send-delivery-confirmation] Order ${orderId} to ${to}`);
    const { data: order } = await supabaseAdmin.from("trips").select("id, order_number, company_id").eq("id", orderId).single();
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: (Deno.env.get("RESEND_FROM_EMAIL") || "noreply@resend.dev").replace(/[<>"']/g, ""), to: [to], subject: subject || `Afleverbevestiging ${order.order_number}`, html: body || `<p>Order ${order.order_number} is afgeleverd.</p>` }),
    });
