...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { email, carrierId, password } = await req.json();
    if (!email || !carrierId) return new Response(JSON.stringify({ error: "email en carrierId verplicht" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    console.log(`[send-carrier-credentials] Sending credentials to ${email} for carrier ${carrierId}`);
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email], subject: "Uw portaal inloggegevens",
        html: `<p>Uw account voor het carrier portaal is aangemaakt.</p><p>E-mail: ${email}</p>${password ? `<p>Wachtwoord: ${password}</p>` : ""}<p>Log in via: <a href="https://rdjlogistics.nl/carrier/login">rdjlogistics.nl/carrier/login</a></p>`,
      }),
    });
    ...
    console.error("[send-carrier-credentials] Error:", err);
