...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: (Deno.env.get("RESEND_FROM_EMAIL") || "noreply@resend.dev").replace(/[<>"']/g, ""), to: [to], subject: subject || `Orderbevestiging ${order.order_number}`, html: body || `<p>Uw order ${order.order_number} is bevestigd.</p>` }),
    });
