...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!invoiceId || !to) return new Response(JSON.stringify({ error: "invoiceId en to verplicht" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    console.log(`[send-purchase-invoice-email] Invoice ${invoiceId} to ${to}`);
    const { data: inv } = await supabaseAdmin.from("purchase_invoices").select("id, invoice_number, company_id").eq("id", invoiceId).single();
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: (Deno.env.get("RESEND_FROM_EMAIL") || "noreply@resend.dev").replace(/[<>"']/g, ""), to: [to], subject: subject || `Inkoopfactuur ${inv.invoice_number}`, html: body || `<p>Bijgaand de inkoopfactuur ${inv.invoice_number}.</p>` }),
    });
