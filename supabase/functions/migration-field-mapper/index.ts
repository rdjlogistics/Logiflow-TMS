...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[migration-field-mapper] Request:`, JSON.stringify(body).slice(0, 200));

    return new Response(JSON.stringify({ success: true, mappings: [], message: "Veldmapping verwerkt" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err) {
