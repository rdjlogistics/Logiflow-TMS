...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[migration-api-sync] Request:`, JSON.stringify(body).slice(0, 200));

    return new Response(JSON.stringify({ success: true, synced: 0, message: "API sync klaar" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err) {
