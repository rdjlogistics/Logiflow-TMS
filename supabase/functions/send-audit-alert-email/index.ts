...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { to, subject, body, alertType, severity } = await req.json();
    ...
    console.log(`[send-audit-alert-email] Alert ${alertType}/${severity} to ${to}`);
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: (Deno.env.get("RESEND_FROM_EMAIL") || "noreply@resend.dev").replace(/[<>"']/g, ""), to: Array.isArray(to) ? to : [to], subject, html: body || `<p>Audit alert: ${alertType} (${severity})</p>` }),
    });
    ...
    console.error("[send-audit-alert-email] Error:", err);
